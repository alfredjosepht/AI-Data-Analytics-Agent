...
import { uploadFile, cleanWorkspace } from "../services/api";
import { Upload, X, ShieldAlert } from "lucide-react";

function UploadPanel({ setUploadInfo, onUploadSuccess }) {
  ...
  const handleRejectCleaning = () => {
    setRecommendations(null);
    alert("Proceeding with raw dataset (minor automatic fixes applied).");
  };
          ...
          <div className="bg-[#0f172a] border border-slate-800 w-full max-w-lg rounded-xl shadow-2xl p-6 relative">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={loading}
                className="flex-1 bg-green-600 hover:bg-green-500 text-white font-medium py-2 rounded-lg transition-colors text-sm"
              >
                Apply Selected Fixes
              </button>
              <button
                onClick={handleRejectCleaning}
                className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium py-2 rounded-lg transition-colors text-sm"
              >
                Skip Major Fixes
              </button>
            </div>
